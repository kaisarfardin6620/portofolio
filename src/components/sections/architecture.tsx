...
import { Brain, Cpu, Database, Globe, Zap } from "lucide-react";
...
// ─── Child component ──────────────────────────────────────────────────────────
// Hooks must be called at the top level of a component, never inside a .map().
// Extracting each node into its own component fixes the Rules-of-Hooks violation.
interface ArchNodeProps {
  node: (typeof nodes)[number];
  scrollYProgress: MotionValue<number>;
}

function ArchNode({ node, scrollYProgress }: ArchNodeProps) {
  ...
  const IconComponent = node.icon as React.ComponentType<{ className?: string }>;
  ...
  return (
    ...
    >
      <div
        className={`flex h-16 w-16 items-center justify-center rounded-xl border border-white/10 bg-white/5 backdrop-blur-md ${node.color} shadow-xl`}
      >
      ...
      {/* Connection Line */}
      <svg className="pointer-events-none absolute -z-10 h-full w-full overflow-visible">
        <motion.line
          x1="50%"
          y1="50%"
          x2={node.x > 0 ? "0%" : "100%"}
...
export function Architecture() {
  ...
  return (
    <section ref={containerRef} className="relative min-h-[120vh] px-6 py-32">
      <div className="mx-auto max-w-5xl">
        ...
        >
          ...
          {/* Exploding Nodes — each rendered via its own component to keep hooks legal */}
          {nodes.map((node) => (
            <ArchNode key={node.id} node={node} scrollYProgress={scrollYProgress} />
          ))}
        </motion.div>
      </div>
    </section>
